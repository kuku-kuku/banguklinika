...
export default function HeroWave({ className = "" }: Props) {
  ...
  // --- Safari detekcija tik runtime (nieko nekeičia kituose browseriuose) ---
  const [isSafari, setIsSafari] = useState(false);
  useEffect(() => {
    if (typeof navigator !== "undefined") {
      const ua = navigator.userAgent;
      // WebKit Safari (ne Chrome/Edge/Opera), taip pat iOS Safari
    ...
    >
      ...
      <div className="relative w-screen left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] overflow-visible">
        <svg
          ...
          overflow="visible"                 // <- paliekam kaip buvo
        >
          <style>{`
          ...
          <path
            ...
            fill="#33ceda"
            fillOpacity="0.6"
            className={pathClass}  // Chrome/Firefox toliau naudos CSS animaciją
            transform="rotate(-180 720 125)" // PALIEKAM KAIP BUVO
          >
            {/* Safari fallback: SMIL animacija, identiškos formos kaip CSS kadrai */}
            {isSafari && (
              <animate
                attributeName="d"
                dur="10s"
