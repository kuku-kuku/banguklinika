...
// Pasiimam būtent antrą pasiūlymą (All-on-4)
const allOn4 = (offers as any).offers[1]

export default function AllOn4Implants() {
  const container = {
  ...
  if (!allOn4) return null

  return (
    <AnimatedSection>
      {/* SEO BŪTENT ŠIAM PUSLAPIUI */}
      <SEO
        title="Pilnas žandikaulio dantų atstatymas ant 4 implantų Klaipėdoje"
        description="Pilnas žandikaulio atkūrimas ant 4 implantų (All-on-4) Klaipėdoje – pažangus metodas, kai atkuriami visi žandikaulio dantys ant 4 implantų. 3D (KPKT) tyrimas, „Straumann“ grupės implantai, originalios detalės ir neišimamas protezas."
      />
      ...
      >
        <motion.header className="mb-6" variants={item}>
          ...
          <h1 className="text-3xl sm:text-4xl font-bold tracking-tight text-darkblue-700">
            Pilnas žandikaulio atkūrimas ant 4 implantų Klaipėdoje
          </h1>
          ...
          <div className="relative grid gap-6 p-6 sm:p-8 md:grid-cols-2 items-stretch">
            ...
            <div className="flex flex-col justify-between">
              <div className="space-y-3">
                ...
                {Array.isArray(allOn4.bulletsTop) && (
                  <ul className="text-slate-700 space-y-1 mt-2">
                    {allOn4.bulletsTop.map((t: string, i: number) => (
                      <li key={i}>• {t}</li>
                    ))}
                  </ul>
                )}
              ...
              <div className="mt-6 pt-4 border-t border-slate-100">
                {(allOn4.priceNow || allOn4.priceWas) && (
                  <>
