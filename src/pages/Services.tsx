...
function AccordionItem({
  id, title, children, openId, setOpenId,
}: {
  id: string
  title: string
...
export default function Services() {
  const sections: Svc[] = useMemo(() => [
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <ul className="list-disc pl-5 space-y-1">
              ...
              <li>Skaitmeninė konstrukcijų modeliavimo technologija – preciziškas pritaikymas.</li>
    ...
    },
    {
      ...
      content: (
        <p>Informacija ruošiama, sugrįžkite artimiausiu metu!</p>
      ),
    },
    {
      id: 'dantu-balinimas',
  ...
  // scroll helper (naudojam scroll-margin, todėl be rankinio offset)
  ...
  // Kai įeini į /paslaugos be hash — viskas uždaryta
  useEffect(() => {
    if (pathname === '/paslaugos' && !hash) setOpenId(null)
  }, [pathname, hash])

  // Pagrindinė logika: atidaryti tikslinę sekciją ir scrollinti
  // Special case: jei jau atidaryta kita — pirma uždarom, palaukiam animacijos, tada atidarom naują ir scrollinam
  useEffect(() => {
    ...
    if (openId && openId !== target) {
      // 1) uždarom seną
      setOpenId(null)
      // 2) po animacijos atidarom naują ir scrollinam
      const t = setTimeout(() => {
        setOpenId(target)
        // leisk framer'ui pradėti plėstis
        requestAnimationFrame(() => {
          const t2 = setTimeout(() => scrollToId(target), 60)
      ...
      // Nėra atidarytos arba ta pati — tiesiog atidaryk ir scrollink
      setOpenId(target)
      requestAnimationFrame(() => {
  ...
  }, [hash]) // sąmoningai neklausom openId čia, kad nesukeltų dvigubo scroll
